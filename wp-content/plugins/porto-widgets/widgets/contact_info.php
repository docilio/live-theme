...
class Porto_Contact_Info_Widget extends WP_Widget {
    ...
    function Porto_Contact_Info_Widget() {
        ...
        $widget_ops = array('classname' => 'contact-info', 'description' => __('Add contact information.', 'porto'));

        $control_ops = array('id_base' => 'contact-info-widget');

        $this->WP_Widget('contact-info-widget', __('Porto: Contact Info', 'porto'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        <div class="contact-info">
            ...
            <ul class="contact-details">
                <?php if ($address) : ?><li><i class="fa fa-map-marker"></i> <strong><?php _e('Address', 'porto') ?>:</strong> <span><?php echo esc_html($address) ?></span></li><?php endif; ?>
                <?php if ($phone) : ?><li><i class="fa fa-phone"></i> <strong><?php _e('Phone', 'porto') ?>:</strong> <span><?php echo esc_html($phone) ?></span></li><?php endif; ?>
                <?php if ($email) : ?><li><i class="fa fa-envelope"></i> <strong><?php _e('Email', 'porto') ?>:</strong> <span><a href="mailto:<?php echo esc_attr($email) ?>"><?php echo esc_html($email) ?></a></span></li><?php endif; ?>
                <?php if ($working) : ?><li><i class="fa fa-clock-o"></i> <strong><?php _e('Working Days/Hours', 'porto') ?>:</strong> <span><?php echo esc_html($working) ?></span></li><?php endif; ?>
            </ul>
    ...
    function form($instance) {
        $defaults = array('title' => __('Contact Us', 'porto'), 'contact_before' => '', 'address' => '1234 Street Name, City Name, Country Name', 'phone' => '(123) 456-7890', 'email' => 'mail@example.com', 'working' => 'Mon - Sun / 9:00 AM - 8:00 PM', 'contact_after' => '');
        ...
        <p>
            <label for="<?php echo $this->get_field_id('address'); ?>">
                <strong><?php echo __('Address', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('phone'); ?>">
                <strong><?php echo __('Phone', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('email'); ?>">
                <strong><?php echo __('Email', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('working'); ?>">
                <strong><?php echo __('Working Days/Hours', 'porto') ?>:</strong>
