...
function porto_twitter_tweets() {
    ...
    $TweetPHP = new TweetPHP(array(
        ...
        'tweet_wrap_close'      => '</li>',
        'error_message'         => __('Oops, our twitter feed is unavailable right now.', 'porto'),
        'error_link_text'       => __('Follow us on Twitter', 'porto'),
        'debug'                 => false
    ));
...
class Porto_Twitter_Tweets_Widget extends WP_Widget {
    ...
    function Porto_Twitter_Tweets_Widget() {
        $widget_ops = array('classname' => 'twitter-tweets', 'description' => __('The most recent tweets from twitter.', 'porto'));

        $control_ops = array('id_base' => 'tweets-widget');

        $this->WP_Widget('tweets-widget', __('Porto: Twitter Tweets', 'porto'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        if ($screen_name && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) {
            ...
            echo '<p>'.__('Please configure widget options.', 'porto').'</p>';
        }
    ...
    function form($instance) {
        $defaults = array('title' => __('Latest Tweets', 'porto'), 'screen_name' => '', 'count' => 2, 'consumer_key' => '', 'consumer_secret' => '', 'access_token' => '', 'access_token_secret' => '');
        ...
        <p>
            <label for="<?php echo $this->get_field_id('consumer_key'); ?>">
                <strong><?php echo __('Consumer Key', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('consumer_secret'); ?>">
                <strong><?php echo __('Consumer Secret', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('access_token'); ?>">
                <strong><?php echo __('Access Token', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('access_token_secret'); ?>">
                <strong><?php echo __('Access Token Secret', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('screen_name'); ?>">
                <strong><?php echo __('Twitter Screen Name', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('count'); ?>">
                <strong><?php echo __('Number of Tweets', 'porto') ?>:</strong>
        ...
        <p><strong><?php echo __('Info', 'porto') ?>:</strong><br/><?php echo __('You can find or create <a href="http://dev.twitter.com/apps" target="_blank">Twitter App here</a>.', 'porto') ?></p>
