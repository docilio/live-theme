...
class Porto_Recent_Portfolios_Widget extends WP_Widget {
    ...
    {
        $widget_ops = array('classname' => 'widget-recent-portfolios', 'description' => __('Show recent portfolios.', 'porto'));

        $control_ops = array('id_base' => 'recent_portfolios-widget');

        $this->WP_Widget('recent_portfolios-widget', __('Porto: Recent Portfolios', 'porto'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        if ($portfolios->have_posts()) :
            ...
            <a class="btn-flat pt-right btn-xs view-more" href="<?php echo get_post_type_archive_link( 'portfolio' ) ?>"><?php _e('View More', 'porto') ?> <i class="fa fa-arrow-<?php echo is_rtl() ? 'left' : 'right' ?>"></i></a>
            <?php
    ...
    {
        $defaults = array('title' => __('Recent Portfolios', 'porto'), 'number' => 6, 'items' => 6, 'view' => 'small', 'cat' => '');
        $instance = wp_parse_args((array) $instance, $defaults); ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>">
                <strong><?php _e('Title', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>">
                <strong><?php _e('Number of portfolios to show', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('view'); ?>">
                <strong><?php _e('View Type', 'porto') ?>:</strong>
                <select class="widefat" id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name('view'); ?>">
                    <option value="small"<?php echo (isset($instance['view']) && $instance['view'] == 'small')? ' selected="selected"' : '' ?>><?php _e('Small', 'porto') ?></option>
                    <option value="large"<?php echo (isset($instance['view']) && $instance['view'] == 'large')? ' selected="selected"' : '' ?>><?php _e('Large', 'porto') ?></option>
                </select>
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('items'); ?>">
                <strong><?php _e('Number of items per slide', 'porto') ?>:</strong>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('cat'); ?>">
                <strong><?php _e('Category IDs', 'porto') ?>:</strong>
