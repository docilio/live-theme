...
function porto_page_title() {
    ...
    if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && $front_page_shop ) ) || is_paged() ) {
        ...
        if ( is_home() ) {
            ...
            echo __( 'Posts tagged &ldquo;', 'woocommerce' ) . single_tag_title('', false) . '&rdquo;';

        } elseif ( is_author() ) {

            $userdata = get_userdata($author);
            echo __( 'Author:', 'woocommerce' ) . ' ' . $userdata->display_name;
...
function porto_main_menu() {
    ...
    if (($header_type == 1 || $header_type == 4) && ($porto_layout == 'wide-left-sidebar' || $porto_layout == 'wide-right-sidebar' || $porto_layout == 'left-sidebar' || $porto_layout == 'right-sidebar') && $porto_settings['menu-sidebar']) {
        if ($is_home || (!$is_home && !$porto_settings['menu-sidebar-home']))
            return '';
    }

    $items_wrap = '<ul id="%1$s" class="%2$s">%3$s</ul>';
    if ($header_type == 1 || $header_type == 4) {
        if ($porto_settings['menu-block']) {
            $items_wrap = '<ul id="%1$s" class="%2$s">%3$s<li class="'. (is_rtl() ? 'pull-left' : 'pull-right').'"><div class="menu-custom-block">'.force_balance_tags($porto_settings['menu-block']).'</div></li></ul>';
        }
    }
...
function porto_sidebar_menu() {
    ...
    if (!((($header_type == 1 || $header_type == 4) && ($porto_layout == 'wide-left-sidebar' || $porto_layout == 'wide-right-sidebar' || $porto_layout == 'left-sidebar' || $porto_layout == 'right-sidebar') && $porto_settings['menu-sidebar']) || $header_type == 'side')) {
        return '';
    }

    if (($header_type == 1 || $header_type == 4) && !$is_home && $porto_settings['menu-sidebar-home'])
        return '';
    $items_wrap = '<ul id="%1$s" class="%2$s">%3$s</ul>';
    if ($porto_settings['menu-block']) {
        $items_wrap = '<ul id="%1$s" class="%2$s">%3$s<li class="menu-custom-item"><div class="menu-custom-block">'.force_balance_tags($porto_settings['menu-block']).'</div></li></ul>';
    }
...
function porto_minicart() {
    ...
    $minicart_type = $porto_settings['header-type'] == 'side' ? 'minicart-inline' : $porto_settings['minicart-type'];
