    ...
    if ( ! class_exists( 'Redux_import_export' ) ) {
        ...
        class Redux_import_export {
            ...
            public function render() {
                ...
                if ( false == $this->is_field ) {
                    echo '<h3>' . 'Import / Export Options' . '</h3>';
                }

                echo '<h4>' . 'Import Options' . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-import-code-button" class="button-secondary">' . 'Import from file' . '</a> <a href="javascript:void(0);" id="redux-import-link-button" class="button-secondary">' . 'Import from URL' . '</a></p>';

                echo '<div id="redux-import-code-wrapper">';
                echo '<p class="description" id="import-code-description">' . apply_filters( 'redux-import-file-description', 'Input your backup file below and hit Import to restore your sites options from a backup.' ) . '</p>';
                ...
                echo '<p class="description" id="import-link-description">' . apply_filters( 'redux-import-link-description', 'Input the URL to another sites options set and hit Import to load the options from that site.' ) . '</p>';
                echo '<input type="text" id="import-link-value" name="' . $this->parent->args['opt_name'] . '[import_link]" class="large-text noUpdate" value="" />';
                echo '</div>';

                echo '<p id="redux-import-action"><input type="submit" id="redux-import" name="' . $this->parent->args['opt_name'] . '[import]" class="button-primary" value="' . 'Import' . '">&nbsp;&nbsp;<span>' . apply_filters( 'redux-import-warning', 'WARNING! This will overwrite all existing option values, please proceed with caution!' ) . '</span></p>';

                echo '<div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>';
                echo '<h4>' . 'Export Options' . '</h4>';
                echo '<div class="redux-section-desc">';

                echo '<p class="description">' . apply_filters( 'redux-backup-description', 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).' ) . '</p>';
                echo '</div>';

                $link = admin_url( 'admin-ajax.php?action=redux_download_options&secret=' . $secret );
                echo '<p><a href="javascript:void(0);" id="redux-export-code-copy" class="button-secondary">' . 'Copy' . '</a> <a href="' . $link . '" id="redux-export-code-dl" class="button-primary">' . 'Download' . '</a> <a href="javascript:void(0);" id="redux-export-link" class="button-secondary">' . 'Copy Link' . '</a></p>';
            ...
            public function render_tab() {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . 'Import / Export' . '</span></a>';
                echo '</li>';
